package com.andoid.connect;

import android.content.Intent;
import android.os.Bundle;
import android.support.v7.app.AppCompatActivity;
import android.util.Log;
import android.view.View;

import com.google.firebase.database.DataSnapshot;
import com.google.firebase.database.DatabaseError;
import com.google.firebase.database.DatabaseReference;
import com.google.firebase.database.FirebaseDatabase;
import com.google.firebase.database.ValueEventListener;

public class ProfileMainPage extends AppCompatActivity {

    private static final String TAG = "";
    Intent intent = getIntent();

    FirebaseDatabase database = FirebaseDatabase.getInstance();
    DatabaseReference myRef = database.getReference("ID1/nation");

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.profile_main_page);
    }

    public void onPAClick(View view) {

        myRef.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                // This method is called once with the initial value and again
                // whenever data at this location is updated.
                String value = dataSnapshot.getValue(String.class);
                Log.d(TAG, "Value is: " + value);

                if (value.equals("Korean")) {
                    Intent kr = new Intent(ProfileMainPage.this,PartnerApplicationKorean.class);
                    startActivity(kr);
                }
                else {
                    Intent foreign = new Intent(ProfileMainPage.this,PartnerApplicationForeign.class);
                    startActivity(foreign);
                }
            }

            @Override
            public void onCancelled(DatabaseError error) {
                // Failed to read value
                Log.w(TAG, "Failed to read value.", error.toException());
            }
        });
    }

    public void onPPClick(View view) {
        Intent intent = new Intent(this, PartnerProfile_main.class);
        startActivity(intent);
    }

    public void OnMesClick(View view){
        Intent intent = getPackageManager().getLaunchIntentForPackage("com.kakao.talk");
        startActivity(intent);
    }

    public void onMypageClick(View view){
        Intent intent = new Intent(this, MyPage.class);
        startActivity(intent);
    }
}
